# Admin List View Developer Guide
...
- You need **list views**, **admin indexes**, **resource tables** as a reusable, package-level boundary.
- You want to consume typed exports from `@platform/admin-listview` instead of reaching into app-specific internals.
...
| `framework/libraries/admin-listview` | Nested publishable library package. |
| `framework/libraries/admin-listview/src` | Public runtime source and exported modules. |
| `framework/libraries/admin-listview/tests` | Unit and contract verification where present. |
...
| --- | --- |
| `index.ts` | `packageId`, `packageDisplayName`, `packageDescription`, `defineListView`, `createListState`, `serializeSavedView`, `deserializeSavedView`, `createListViewFromResource` |
...
- Hosts should import from `@platform/admin-listview`, not deep internal file paths, so refactors inside `src/` do not become accidental breaking changes.
...
```mermaid
graph TD
  host["Host app or plugin"] --> import["Import from @platform/admin-listview"]
...
console.log(typeof defineListView);
```
